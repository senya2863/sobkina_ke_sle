...
convergence_csv = os.path.join(build_dir, 'convergence_hw9.csv')
timing_csv = os.path.join(build_dir, 'timing_hw9.csv')
output_convergence = os.path.join(project_root, 'convergence_hw9.png')
output_time = os.path.join(project_root, 'convergence_time_hw9.png')

plt.rcParams['figure.figsize'] = (10, 6)
plt.rcParams['font.size'] = 11

methods = {
    'CG_plain_error': {'color': '#1f77b4', 'label': 'CG (plain)'},
    'CG_prec_error': {'color': '#d62728', 'label': 'CG + IC(0)'}
}
...
plt.title('CG with and without IC(0) Preconditioner', fontsize=14, fontweight='bold')
...
if os.path.exists(timing_csv):
    ...
    plt.ylabel('Error', fontsize=12)
    plt.title('Convergence vs Time (HW9)', fontsize=14, fontweight='bold')
    plt.legend(loc='upper right', fontsize=10)
    plt.grid(True, which='both', alpha=0.6, linestyle='--')
    plt.tight_layout()
