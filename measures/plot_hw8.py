...
convergence_csv = os.path.join(build_dir, 'convergence_hw8.csv')
timing_csv = os.path.join(build_dir, 'timing_hw8.csv')
output_convergence = os.path.join(project_root, 'convergence_hw8.png')
output_time = os.path.join(project_root, 'convergence_time_hw8.png')
...
methods = {
    ...
    'CG_error': {'color': '#2ca02c', 'label': 'CG'},
    'GMRES_error': {'color': '#d62728', 'label': 'GMRES'}
}
...
plt.title('Convergence Comparison (HW8 - GMRES)', fontsize=14, fontweight='bold')
...
if os.path.exists(timing_csv):
    ...
    plt.ylabel('Error', fontsize=12)
    plt.title('Convergence vs Time (HW8)', fontsize=14, fontweight='bold')
    plt.legend(loc='upper right', fontsize=10)
    plt.grid(True, which='both', alpha=0.6, linestyle='--')
    plt.tight_layout()
