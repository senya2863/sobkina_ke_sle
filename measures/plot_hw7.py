...
convergence_csv = os.path.join(build_dir, 'convergence_hw7.csv')
timing_csv = os.path.join(build_dir, 'timing_hw7.csv')
output_convergence = os.path.join(project_root, 'convergence_hw7.png')
output_time = os.path.join(project_root, 'convergence_time_hw7.png')

plt.rcParams['figure.figsize'] = (10, 6)
plt.rcParams['font.size'] = 11

methods = {
    'GS_error': {'color': '#1f77b4', 'label': 'Gauss-Seidel'},
    'SOR_error': {'color': '#ff7f0e', 'label': 'SOR'},
    'SORCheb_error': {'color': '#2ca02c', 'label': 'SOR+Cheb'},
    'SD_error': {'color': '#d62728', 'label': 'Steepest Descent'},
    'CG_error': {'color': '#9467bd', 'label': 'CG'}
}
...
plt.title('Convergence Comparison (HW7)', fontsize=14, fontweight='bold')
...
if os.path.exists(timing_csv):
    ...
    plt.ylabel('Error', fontsize=12)
    plt.title('Convergence vs Time (HW7)', fontsize=14, fontweight='bold')
    plt.legend(loc='upper right', fontsize=10)
    plt.grid(True, which='both', alpha=0.6, linestyle='--')
    plt.tight_layout()
